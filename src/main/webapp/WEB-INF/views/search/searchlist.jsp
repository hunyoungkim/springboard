...
<c:out value="${search.subject}"/></a>        
 ...
 <td><c:out value="${search.read_count}"/></td>      
  ...
  <c:otherwise>  
  <a href="./BoardSearchList.qa?page=${page-1}&keyword=${keyword}&keyfield=${keyfield}">[이전]</a>&nbsp; 
  </c:otherwise>       
  </c:choose> 
  <c:forEach var="start" begin="${startpage}" end="${endpage}" step="1"> 
  <c:choose>   
  <c:when test="${start eq page}">[<c:out value="${start}" />]</c:when>   
  <c:otherwise> <a href="./BoardSearchList.qa?page=${start}&keyword=${keyword}&keyfield=${keyfield}">[<c:out value= "${start}" />]</a>&nbsp;    
  </c:otherwise>
   </c:choose> 
   </c:forEach>         
   <c:choose>         
    ...
    <c:otherwise>     
    <a href="./BoardSearchList.qa?page=${page+1}&keyword=${keyword}&keyfield=${keyfield}">[다음]</a>           
    </c:otherwise>        
    </c:choose>        
    </td>      
    </tr>    
